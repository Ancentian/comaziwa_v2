<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form id="edit_leave_form"  action="{{ url('leaves/update', [$leave->id]) }}" method="POST">
                @csrf
                <div class="form-group">
                    ...
                    <select class="select form-control" name="employee_id" id="employeeID">
                        ...
                        <option value="{{$key->id}}" {{ $key->id === $leave->employee_id ? 'selected' : '' }}>{{$key->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="select form-control" name="type" id="leaveType">
                            @foreach($leave_types as $key)
                                <option value="{{$key->id}}" {{ $key->id == $leave->type ? 'selected' : '' }}>{{$key->type_name}}</option>
                            @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <div class="cal-icon">
                        <input class="form-control datetimepicker" name="date_from" value="{{$leave->date_from}}" type="text">
                    </div>
                </div>
                <div class="form-group">
                    <label>To <span class="text-danger">*</span></label>
                    <div class="cal-icon">
                        <input class="form-control datetimepicker" name="date_to" value="{{$leave->date_to}}" type="text">
                    </div>
                </div>
                <div class="form-group">
                        <label>Remaining Days <span class="text-danger">*</span></label>
                        <input class="form-control" name="remaining_days" id="rem_days" readonly value="{{$leave->remaining_days}}" type="text">
                    </div>
                <div class="form-group">
                    <label>Leave Reason <span class="text-danger">*</span></label>
                    <textarea rows="4" class="form-control" name="reasons">{{$leave->reasons}}</textarea>
                </div>
