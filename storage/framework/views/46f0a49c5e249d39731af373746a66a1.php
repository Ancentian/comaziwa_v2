<script>
    ...
    $('#taxband').on('submit', function (e) {
        ...
        $.ajax({
            url: '<?php echo e(url('taxbands/store_taxbands')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                // Handle success response
    ...
    //Save Benefit in Kind
    $('#benefitsInKind').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: '<?php echo e(url('benefits/store_benefitsInKind')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                // Handle success response
    ...
    $('#add_project_form').on('submit', function (e) {
        ...
        $.ajax({
            url: '<?php echo e(url('projects/store')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                // Handle success response
    ...
    //Save Staff Project Data
    $('#add_staff_project_form').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: '<?php echo e(url('staff/store_staff_project')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                // Handle success response
                ...
                // Close the modal
                $('#create_staff_project').modal('hide');
                toastr.success(response.message, 'Success');
                $(".submit-btn").prop('disabled', false); 
                $(".submit-btn").html("Submit");
            },
    ...
    $('#add_task_form').on('submit', function (e) {
        ...
        $.ajax({
            url: '<?php echo e(url('tasks/store')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                form.reset();
    ...
    //Add Staff Task
    $('#add_staff_task_form').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: '<?php echo e(url('staff/store-staff-task')); ?>',
            method: 'POST',
            data: formData,
            success: function (response) {
                form.reset();
...
$('#attendance_form').on('submit', function(e) {
    ...
    var form = this;
    var formData = $(this).serialize();
    var punchButton = $('#punch_button');
    //console.log(punchButton);

     // Function to update the button text
     function updateButtonText(punchedIn, punchedOut) {
    ...
    $.ajax({
        url: '<?php echo e(url('attendance/checkPunchStatus')); ?>',
        method: 'GET',
        data: formData,
        success: function(response) {
            var punchedIn = response.punchedIn;
            var punchedOut = response.punchedOut;
            // Update the button text
            updateButtonText(punchedIn, punchedOut);
            
            //console.log(punchedOut);

            if (punchedIn && !punchedOut) {
                // User is already punched in, so this is a punch-out action
                punchButton.text('Punching Out...');

                localStorage.removeItem('startTime');

                // Perform the AJAX request to punch out
                $.ajax({
                    url: '<?php echo e(url('attendance/store_punchOut')); ?>',
                    method: 'POST',
                    data: formData,
                    success: function(response) {
                        punchButton.text('Punch In'); // Update the button text
                        form.reset();
                        toastr.success(response.message, 'Success');
                        // Close the modal
                        window.location.reload();
                        // Additional logic or notifications for successful punch-out
                    },
                    error: function(xhr, status, error) {
                ...
                // User is not punched in or already punched out, so this is a punch-in action
                punchButton.text('Punching In...');

                // Perform the AJAX request to punch in
                $.ajax({
                    url: '<?php echo e(url('attendance/store_punchIn')); ?>',
                    method: 'POST',
                    data: formData,
                    success: function(response) {
                        updateButtonText(punchedIn, punchedOut);
                        ...
                        // Close the modal
                        window.location.reload();
                        // Additional logic or notifications for successful punch-in
                    },
                    error: function(xhr, status, error) {
                        var responseJSON = xhr.responseJSON;
                        if (responseJSON && responseJSON.errors) {
...
$('#add_training_form').on('submit', function (e) {
    ...
    $.ajax({
        url: '<?php echo e(url('trainings/store-training')); ?>',
        method: 'POST',
        data: formData,
        success: function (response) {
            form.reset();
...
$('#add_agent_form').on('submit', function (e) {
    ...
    $.ajax({
        url: '<?php echo e(url('superadmin/create-agent')); ?>',
        method: 'POST',
        data: formData,
        success: function (response) {
            form.reset();
    ...
    $('#modal_delete_button').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if ($('#salary_types_table').length > 0) {
                    salary_types_table.ajax.reload();
                }
...
$(document).ready(function(){
    ...
    // load Tax Bands Data
    $(document).ready(function(){
        tax_bands_table = $('#tax_bands_table').DataTable({
            <?php echo $__env->make('layout.export_buttons', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            processing: true,
            serverSide: false,
            ajax: {
                url : "<?php echo e(url('taxbands/list')); ?>",
                data: function(d){
    ...
    $(document).ready(function(){
        deductions_table = $('#deductions_table').DataTable({
            ...
            serverSide: false,
            ajax: {
                url : "<?php echo e(url('deductions/list')); ?>",
                data: function(d){
...
</script><?php /**PATH C:\laragon\www\payroll\resources\views/layout/scripts.blade.php ENDPATH**/ ?>