...
$(document).ready(function() {
    $.ajax({
        ...
        success: function(data) {
            if (data.length === 0) {
                $('.col-md-6.text-center').hide();
                return;
            }

            var isEmpty = data.some(item => item.total_morning === 0 && item.total_evening === 0 && item.total_rejected === 0 && item.total_milk === 0);
            if (isEmpty) {
                $('.col-md-6.text-center').hide();
                return;
            }

            $('.col-md-6.text-center').show();
...
$(document).ready(function() {
    $.ajax({
        ...
        success: function(data) {
            ...
            var isEmpty = data.some(item => item.total_milk === 0);
            if (isEmpty) {
                $('.col-md-6.text-center').hide();
                return;
            }
...
$(document).ready(function() {
    $.ajax({
        ...
        success: function(data) {
            ...
            var isEmpty = data.every(item => item.total_milk === 0);
            if (isEmpty) {
                $('.col-md-6.text-center').hide();
                return;
            }
